...
/**
 * ADR-029: buyout_outcome отдаёт признаки для созревания когорты —
 * наблюдалось ли закрытие отправления и находится ли строка в доставке.
 * Колонки добавляются в конец, форма остальных не меняется.
 */
final class Version20260925150000 extends AbstractMigration
{
    public function getDescription(): string
    {
        return 'Expose observed resolution and in-flight flags on buyout_outcome for cohort maturity';
    }

    public function up(Schema $schema): void
    {
        $this->addSql("SET LOCAL lock_timeout = '5s'");
        $this->addSql(self::viewSql(maturityEvidence: true));
    }

    public function down(Schema $schema): void
    {
        // CREATE OR REPLACE VIEW не удаляет колонки: прежняя форма
        // восстанавливается пересозданием, зависимых объектов у view нет.
        $this->addSql("SET LOCAL lock_timeout = '5s'");
        $this->addSql('DROP VIEW buyout_outcome');
        $this->addSql(self::viewSql(maturityEvidence: false));
    }

    private static function viewSql(bool $maturityEvidence): string
    {
        // Закрытие наблюдалось, если до первого terminal-наблюдения было
        // хотя бы одно более раннее наблюдение того же posting. Иначе
        // resolved_at — начало наблюдения, а не момент закрытия.
        $resolutionObserved = $maturityEvidence ? <<<'SQL'
            ,
                                   COALESCE(BOOL_OR(
            ...
            SQL : '';
        $salesResolutionObserved = $maturityEvidence ? ',
                       e.resolution_observed' : '';
        $maturityColumns = $maturityEvidence ? <<<'SQL'
            ,
            ...
            status_evidence AS NOT MATERIALIZED (
                SELECT s.company_id, s.marketplace_account_id, s.posting_number,
                       ...
                       COALESCE(BOOL_OR(
                           ...
                           AND (s.observed_at, s.raw_document_id) < (t.resolved_at, t.resolved_raw_document_id)
                       ), FALSE) AS has_pre_handover_observation{$resolutionObserved}
        ...
        return <<<SQL
            ...
            sales_evidence AS NOT MATERIALIZED (
                SELECT s.company_id, s.marketplace_account_id, s.source_row_id,
                       ...
                       e.handed_over_at, e.resolved_at, e.has_pre_handover_observation{$salesResolutionObserved}
                FROM sales_fact s
            ...
            SELECT c.company_id, c.marketplace_account_id, c.source_row_id,
                   ...
                   ) AS is_forecast_eligible{$maturityColumns}
            FROM classified c
            CROSS JOIN LATERAL (
