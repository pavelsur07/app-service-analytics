...
final class Version20260901090000 extends AbstractMigration
{
    ...
    {
        $this->addSql('CREATE TABLE marketplace_posting_status (company_id UUID NOT NULL, marketplace_account_id UUID NOT NULL, posting_number VARCHAR(64) NOT NULL, raw_document_id UUID NOT NULL, order_number VARCHAR(64) NOT NULL, status VARCHAR(32) NOT NULL, substatus VARCHAR(64) DEFAULT NULL, cancel_reason_id BIGINT DEFAULT NULL, observed_at TIMESTAMP(0) WITHOUT TIME ZONE NOT NULL, PRIMARY KEY (company_id, marketplace_account_id, posting_number, raw_document_id))');
        $this->addSql('CREATE INDEX idx_marketplace_posting_status_effective ON marketplace_posting_status (company_id, marketplace_account_id, posting_number, observed_at, raw_document_id)');
        $this->addSql('CREATE INDEX idx_marketplace_posting_status_maturity ON marketplace_posting_status (company_id, marketplace_account_id, observed_at, status)');
        $this->addSql('CREATE INDEX idx_marketplace_posting_status_raw ON marketplace_posting_status (company_id, raw_document_id)');
        ...
        $this->addSql('ALTER TABLE sales_fact ADD posting_number VARCHAR(64) DEFAULT NULL');
        $this->addSql('ALTER TABLE sales_fact ADD order_number VARCHAR(64) DEFAULT NULL');
        ...
        $this->addSql('CREATE TABLE marketplace_return_fact (company_id UUID NOT NULL, marketplace_account_id UUID NOT NULL, source_row_id TEXT NOT NULL, order_number VARCHAR(64) NOT NULL, marketplace_sku VARCHAR(64) NOT NULL, return_type VARCHAR(64) NOT NULL, return_reason_name TEXT NOT NULL, posting_number VARCHAR(64) NOT NULL, source_id BIGINT NOT NULL, quantity INT NOT NULL, visual_status_id INT NOT NULL, visual_status VARCHAR(64) NOT NULL, visual_status_changed_at TIMESTAMP(0) WITHOUT TIME ZONE NOT NULL, raw_document_id UUID NOT NULL, row_hash CHAR(64) NOT NULL, first_loaded_at TIMESTAMP(0) WITHOUT TIME ZONE NOT NULL, last_updated_at TIMESTAMP(0) WITHOUT TIME ZONE NOT NULL, PRIMARY KEY (company_id, marketplace_account_id, source_row_id), CONSTRAINT chk_marketplace_return_fact_quantity CHECK (quantity > 0))');
        $this->addSql('CREATE INDEX idx_return_fact_order_sku ON marketplace_return_fact (company_id, marketplace_account_id, order_number, marketplace_sku)');
        $this->addSql('CREATE INDEX idx_return_fact_visual_changed ON marketplace_return_fact (company_id, visual_status_changed_at)');
        $this->addSql('CREATE INDEX idx_return_fact_raw_document ON marketplace_return_fact (company_id, raw_document_id)');

        $this->addSql("CREATE TABLE ozon_return_reason_classification (return_type VARCHAR(64) NOT NULL, return_reason_name TEXT NOT NULL, event_stage VARCHAR(32) NOT NULL, verified_at DATE NOT NULL, PRIMARY KEY (return_type, return_reason_name), CONSTRAINT chk_ozon_return_event_stage CHECK (event_stage IN ('HANDOVER_REFUSAL', 'PICKUP_EXPIRED', 'DELIVERY_FAILED', 'CANCELLED'))) ");
        $this->addSql(<<<'SQL'
        ...
        $this->addSql(<<<'SQL'
            ...
            base AS NOT MATERIALIZED (
                SELECT s.company_id, s.marketplace_account_id, s.source_row_id,
                       ...
                       COALESCE(r.client_return_quantity, 0) AS client_return_quantity,
                       COALESCE(r.handover_refusal_quantity, 0) AS handover_refusal_quantity,
                       COALESCE(r.pickup_expired_quantity, 0) AS pickup_expired_quantity,
                       COALESCE(r.delivery_failed_quantity, 0) AS delivery_failed_quantity,
                       COALESCE(r.cancelled_quantity, 0) AS cancelled_quantity
                FROM sales_fact s
            ...
            classified_outcome AS NOT MATERIALIZED (
                SELECT b.*,
                   CASE
                       WHEN b.status_is_known AND b.status = 'delivered' AND b.client_return_quantity > 0 THEN 'R'
                       WHEN b.status_is_known AND b.status = 'delivered' THEN 'D'
                       WHEN b.status_is_known AND b.status = 'cancelled' AND b.handover_refusal_quantity > 0 THEN
                           CASE
                               WHEN b.order_number IS NOT NULL AND o.has_delivered THEN 'P'
                               WHEN b.order_number IS NOT NULL AND o.all_terminal THEN 'T2'
                               ELSE NULL
                           END
                       WHEN b.status_is_known AND b.status = 'cancelled'
                            AND (b.handed_over_at IS NOT NULL OR b.pickup_expired_quantity > 0 OR b.delivery_failed_quantity > 0) THEN 'T2'
                       WHEN b.status_is_known AND b.status = 'cancelled'
                            AND b.cancelled_quantity > 0
                            AND b.has_pre_handover_observation
                            AND b.handed_over_at IS NULL THEN 'T1'
                       ELSE NULL
                   END::VARCHAR(2) AS classified_outcome
                FROM base b
                LEFT JOIN LATERAL (
                ...
                ) o ON b.order_number IS NOT NULL
            ),
            classified AS NOT MATERIALIZED (
                SELECT c.*,
                       CASE
                           WHEN c.classified_outcome = 'R' THEN LEAST(c.quantity::bigint, c.client_return_quantity)
                           WHEN c.classified_outcome = 'D' THEN c.quantity::bigint
                           WHEN c.classified_outcome IN ('P', 'T2') AND c.handover_refusal_quantity > 0
                               THEN LEAST(c.quantity::bigint, c.handover_refusal_quantity)
                           WHEN c.classified_outcome = 'T2' AND c.handed_over_at IS NOT NULL THEN c.quantity::bigint
                           WHEN c.classified_outcome = 'T2'
                               THEN LEAST(c.quantity::bigint, c.pickup_expired_quantity + c.delivery_failed_quantity)
                           WHEN c.classified_outcome = 'T1' THEN LEAST(c.quantity::bigint, c.cancelled_quantity)
                           ELSE c.quantity::bigint
                       END AS classified_quantity
                FROM classified_outcome c
            )
            SELECT c.company_id, c.marketplace_account_id, c.source_row_id,
                   ...
                   c.handed_over_at,
                   c.resolved_at,
                   (c.is_active_pending AND allocation.outcome IS NULL) AS is_forecast_eligible
            FROM classified c
            CROSS JOIN LATERAL (
                SELECT c.classified_outcome AS outcome, c.classified_quantity AS quantity
                WHERE c.classified_quantity > 0
                UNION ALL
                SELECT CASE WHEN c.classified_outcome = 'R' THEN 'D' ELSE NULL END AS outcome,
                       c.quantity::bigint - c.classified_quantity AS quantity
                WHERE c.quantity::bigint > c.classified_quantity
            ) allocation
            SQL);
    }
    ...
    {
        $this->addSql('DROP VIEW buyout_outcome');
        $this->addSql('DROP TABLE ozon_return_reason_classification');
        $this->addSql('DROP TABLE marketplace_return_fact');
        $this->addSql('DROP INDEX CONCURRENTLY idx_marketplace_raw_document_ozon_history');
        $this->addSql('DROP TABLE marketplace_posting_status');
        $this->addSql('DROP INDEX CONCURRENTLY idx_sales_fact_posting');
        $this->addSql('DROP INDEX CONCURRENTLY idx_sales_fact_order_sku');
        $this->addSql('ALTER TABLE sales_fact DROP posting_number');
        $this->addSql('ALTER TABLE sales_fact DROP order_number');
    }
}
