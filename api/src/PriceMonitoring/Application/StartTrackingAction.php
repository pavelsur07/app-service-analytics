...
final class StartTrackingAction
{
    ...
    public function __invoke(
        ...
        // Считаются другие артикулы, не все: иначе повторная отправка
        // пятидесятого — та самая, ради которой включение сделано
        // идемпотентным, — упиралась бы в потолок и получала отказ
        // на действие, которое в прошлый раз прошло. Потолок обязан
        // отсекать новый артикул, а не повтор уже принятого.
        //
        // Проверка перед записью здесь допустима и §4 не нарушает:
        // это не защита от дубля (её держит уникальный индекс), а мягкая
        // граница. Две одновременные попытки могут дать пятьдесят первый
        // артикул — безвредно; замок ради этого стоил бы дороже.
        if ($this->trackedSkus->countActiveExcluding($companyId, $marketplaceSku) >= self::MAX_TRACKED) {
