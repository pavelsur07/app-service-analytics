 ...
 * признак «сообщение знает компанию» у самих сообщений. **Пробел уже
 * не теоретический**: первая запись из воркера — предупреждение перед
 * переводом Ozon-подключения в broken (`OzonAccountBrokenLogger`,
 * вызывается из `FetchOzon{Catalog,Postings,Expenses,Returns}Handler`) —
 * и в ней `company_id` и `marketplace_account_id` кладутся явными полями
 * контекста вызывающим кодом, а не этим процессором. Общее решение
 * (middleware очереди) остаётся не сделанным; до него каждый новый
 * воркер, который начинает писать в журнал, обязан класть `company_id`
 * в контекст сам, как это делает `OzonAccountBrokenLogger`.
 *
