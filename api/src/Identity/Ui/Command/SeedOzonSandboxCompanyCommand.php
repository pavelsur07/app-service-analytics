...
/**
 * Заводит компанию и Ozon-подключение для сидирования локальной
 * песочницы и сквозного сценария `bin/e2e-seed.sh`. Продуктовым способом
 * завести учётку это перестало быть в момент выкладки HTTP-онбординга
 * (ADR-021) — саморегистрация и подключение кабинета через UI работают
 * в проде.
 *
 * Аудит-запись о подключении, в отличие от HTTP-сценария, не пишется:
 * у оператора, запускающего эту команду в терминале, нет актора, который
 * можно было бы указать в `AuditRecord` — ни пользователя, ни
 * администратора. Это сознательное сужение области действия сценария
 * до сидирования, а не пробел в аудите продуктового пути. Условие
 * пересмотра — ADR-023.
 */
#[AsCommand(
