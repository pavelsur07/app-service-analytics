<?php

declare(strict_types=1);

namespace App\Identity\Infrastructure\Repository;

use App\Identity\Domain\EmailVerificationUserByEmailQuery;
use App\Identity\Domain\User;
use Doctrine\ORM\EntityManagerInterface;

/** Реализация exact pre-auth capability из CLAUDE.md §1 и ADR-021. */
final readonly class DoctrineEmailVerificationUserByEmailQuery implements EmailVerificationUserByEmailQuery
{
    public function __construct(
        private EntityManagerInterface $entityManager,
    ) {
    }

    public function findForResend(string $email): ?User
    {
        $user = $this->entityManager->createQueryBuilder()
            ->select('u')
            ->from(User::class, 'u')
            ->where('u.email = :email')
            ->setParameter('email', User::normalizeEmail($email))
            ->getQuery()
            ->getOneOrNullResult();

        \assert(null === $user || $user instanceof User);

        return $user;
    }
}
