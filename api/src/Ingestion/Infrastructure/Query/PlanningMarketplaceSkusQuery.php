...
final readonly class PlanningMarketplaceSkusQuery
{
    ...
    /** @param list<string> $skus */
    public function knownDetails(string $companyId, string $accountId, array $skus): QueryBuilder
    {
        $source = <<<'SQL'
            (
                SELECT l.marketplace_sku, l.offer_id, l.name
                FROM marketplace_listing l
                WHERE l.company_id = :company AND l.marketplace_account_id = :account AND l.marketplace_sku IN (:skus)
                UNION ALL
                SELECT DISTINCT s.marketplace_sku, NULL::varchar AS offer_id, NULL::varchar AS name
                FROM sales_fact s
                WHERE s.company_id = :company AND s.marketplace_account_id = :account AND s.marketplace_sku IN (:skus)
                  AND NOT EXISTS (
        ...
        return $this->connection->createQueryBuilder()->select('marketplace_sku', 'offer_id', 'name')->from($source)
            ->setParameter('company', $companyId)->setParameter('account', $accountId)
            ->setParameter('skus', $skus, ArrayParameterType::STRING)
            ->orderBy('marketplace_sku')->setMaxResults(\count($skus));
    }
