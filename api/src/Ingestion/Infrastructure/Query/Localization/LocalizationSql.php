 ...
 * в периоде, кроме отменённых. Локальная — cluster_from = cluster_to.
...
final class LocalizationSql
{
    ...
    public static function linesCte(): string
    {
        ...
        return <<<SQL
            ...
            lines AS (
                SELECT s.marketplace_sku, s.quantity, s.cluster_from, s.cluster_to,
                       (s.cluster_from IS NOT NULL AND s.cluster_to IS NOT NULL) AS has_clusters,
                       COALESCE(s.cluster_from = s.cluster_to, false) AS is_local,
                       l.forward_cost_minor, l.reverse_cost_minor, l.currency_min, l.currency_max
                FROM sales s
    ...
    public static function metricsSelect(): string
    {
        return <<<'SQL'
            ...
            ROUND(10000::numeric * SUM(quantity) FILTER (WHERE forward_cost_minor IS NOT NULL)
                  / NULLIF(SUM(quantity), 0))::int AS charged_share_bps,
