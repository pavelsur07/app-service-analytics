 ...
 *
 * Метаданные всех документов — одним запросом по списку id (CLAUDE.md §6),
 * до открытия транзакции замены дня; тела — по одному, лениво, из базы или
 * объектного хранилища (ADR-024). Так в памяти одна пачка, а под
 * блокировкой отметки дня — только чтение тел, без запросов к своей БД.
