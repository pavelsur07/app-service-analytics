 ...
 * (RawDocumentBody::COLUMNS), и у документов, чьё тело лежит в базе, оно
 * приходит сразу — память растёт с числом таких пачек. Это документы,
 * записанные в аварийный период ADR-025, в том числе дедуплицированные
 * повторы того же дня после выхода из него: RawDocumentBody читает тело
 * из базы первым. Читать их из базы по одному значило бы запросы к своей
 * БД в цикле (§6). У документов с телом в S3 — в памяти одна пачка;
 * тела в базе — временное состояние, а каталоги на этой стадии — десятки SKU.
