...
final readonly class IngestionPlanningFacade
{
    ...
    public function knownMarketplaceSkus(string $companyId, string $marketplaceAccountId, array $marketplaceSkus): array
    {
        ...
        $rows = $this->queryGuard->read(fn (): array => $this->skus->known($companyId, $marketplaceAccountId, $marketplaceSkus)->executeQuery()->fetchFirstColumn());
        foreach ($rows as $row) {
            if (!\is_string($row)) {
    ...
    public function knownMarketplaceSkusForPlanImport(string $companyId, string $marketplaceAccountId, array $marketplaceSkus): array
    {
        ...
        $rows = $this->queryGuard->read(
            fn (): array => $this->skus->known($companyId, $marketplaceAccountId, $marketplaceSkus)->executeQuery()->fetchFirstColumn(),
            '25s',
            false,
        );
        foreach ($rows as $row) {
    ...
    public function knownMarketplaceSkuDetailsForPlanImport(string $companyId, string $marketplaceAccountId, array $marketplaceSkus): array
    {
        ...
        $rows = $this->queryGuard->read(
            fn (): array => $this->skus->knownDetails($companyId, $marketplaceAccountId, $marketplaceSkus)->executeQuery()->fetchAllAssociative(),
            '25s',
            false,
        );
