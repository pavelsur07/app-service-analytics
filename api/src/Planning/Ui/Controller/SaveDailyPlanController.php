...
use App\Planning\Ui\Request\SaveDailyPlanRequest;
...
#[Route(
    ...
    name: 'planning_daily_plan_save',
    requirements: ['companyId' => Requirement::UUID, 'accountId' => Requirement::UUID, 'sku' => '.+'],
    methods: ['PUT'],
)]
final readonly class SaveDailyPlanController
{
    public function __construct(
        private SaveDailyPlanAction $save,
        private ValidatorInterface $validator,
    ) {
    }

    #[OA\RequestBody(required: true, content: new OA\JsonContent(required: ['quantity', 'expectedVersion'], properties: [new OA\Property(property: 'quantity', type: 'integer', maximum: DailyPlan::MAX_QUANTITY, minimum: 0), new OA\Property(property: 'expectedVersion', type: 'integer', maximum: DailyPlan::MAX_MUTABLE_VERSION, minimum: 0)], additionalProperties: false))]
    #[OA\Response(response: 200, description: 'План сохранён', content: new Model(type: DailyPlanItemResponse::class))]
    ...
    {
        try {
            $input = SaveDailyPlanRequest::fromJson($request->getContent());
            $sku = DailyPlanParameters::sku($sku);
            $businessDate = DailyPlanParameters::date($date);
        } catch (\InvalidArgumentException $invalid) {
            return $this->error(Response::HTTP_UNPROCESSABLE_ENTITY, $invalid->getMessage(), 'Проверьте количество, версию, SKU и дату.');
        }
        if (0 !== \count($this->validator->validate($input))) {
            return $this->error(Response::HTTP_UNPROCESSABLE_ENTITY, 'request_invalid', 'Проверьте количество и версию плана.');
        }
        $actorId = $request->attributes->get(RequestAttributes::ActorUserId);
        \assert(\is_string($actorId));

        return $this->respond(($this->save)($companyId, $accountId, $sku, $businessDate, $input->validQuantity(), $input->validExpectedVersion(), $actorId));
    }

    private function respond(DailyPlanMutationResult $result): JsonResponse
    {
        return match ($result->outcome) {
            DailyPlanMutationOutcome::Saved => new JsonResponse(DailyPlanItemResponse::fromItem($result->current ?? throw new \LogicException('Нет сохранённого плана.'))),
