 ...
 * граница доверия. Расчёт проверяет BuildLocalizationReportActionTest.
 */
final class ShowLocalizationReportControllerTest extends WebTestCase
{
    public function testReportContainsOnlyTheRequestedCompanyData(): void
    {
        ...
        $client->request('GET', '/api/companies/'.$companyId->toRfc4122().'/localization?days=30');

        self::assertResponseIsSuccessful();
        $payload = $this->payload($client);
        self::assertIsArray($payload['summary']);
        self::assertSame(3, $payload['summary']['quantity']);
    ...
    public function testCompanyMemberCannotReadAnotherCompany(): void
    {
        ...
        $client->request('GET', '/api/companies/'.Uuid::v7()->toRfc4122().'/localization?days=30');
    ...
    public function testInvalidQueryReturnsStable422(string $query, string $code): void
    {
        ...
        $client->request('GET', '/api/companies/'.$companyId->toRfc4122().'/localization?'.$query);
