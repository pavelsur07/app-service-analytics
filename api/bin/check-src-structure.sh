...
# PriceMonitoring, Links, Planning и Kernel.php — ничего сверх спроектированных модулей
...
for entry in src/*; do
    name="$(basename "$entry")"
    case "$name" in
        Shared|Identity|Ingestion|PriceMonitoring|Links|Planning|Kernel.php) ;;
        *) unexpected="$unexpected $name" ;;
    esac
done

if [ -n "$unexpected" ]; then
    echo "api/src/ содержит лишнее (не Shared/Identity/Ingestion/PriceMonitoring/Links/Planning/Kernel.php):" >&2
...
echo "OK: api/src/ содержит только Shared, Identity, Ingestion, PriceMonitoring, Links, Planning, Kernel.php"
