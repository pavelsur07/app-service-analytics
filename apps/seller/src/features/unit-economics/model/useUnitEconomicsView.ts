...
export function useUnitEconomicsView(): {
  ...
  const write = (patch: Record<string, string>): void => {
    ...
    // Стопка гасится явно, а не одним лишь расхождением ключа. Иначе
    // возврат к прежней сортировке (A → B → A) воскрешал бы её: ключ
    // снова совпал бы, и вместо первой страницы клиент оказывался бы
    // на той, где был до переключения. Пустой ключ не совпадает
    // ни с одним представлением, поэтому следующий рендер начнёт
    // с начала при любом patch.
