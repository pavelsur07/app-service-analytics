...
const BY_CODE: Record<string, ReplaceCredentialsFailure> = {
  ...
  credentials_rejected_sales: {
    ...
    description:
      'Включите доступ к отправлениям (FBO/FBS) в кабинете продавца и выпустите ключ заново. Старый ключ остался на месте — синхронизация не изменилась.',
    refetch: false,
  },
  credentials_rejected_expenses: {
    title: 'Ключу не хватает права на финансы',
    description:
      'Включите доступ к финансовым отчётам в кабинете продавца и выпустите ключ заново. Старый ключ остался на месте — синхронизация не изменилась.',
    refetch: false,
  },
  credentials_rejected_returns: {
    title: 'Ключу не хватает права на возвраты',
    description:
      'Включите доступ к возвратам в кабинете продавца и выпустите ключ заново. Старый ключ остался на месте — синхронизация не изменилась.',
    refetch: false,
  },
  ...
  marketplace_unavailable: {
    ...
    description:
      'Ключ выпускать не нужно — с ключами всё в порядке. Старый ключ остался на месте, синхронизация не изменилась. Повторите через несколько минут.',
    // Подключение не изменилось: сохранение даже не началось, поэтому
    // список подключений устареть не мог и перечитывать его незачем —
    // в отличие от revoked/version_conflict/not_found, где состояние
    // на сервере уже другое, чем то, что видит форма.
    refetch: false,
  },
  connection_revoked: {
