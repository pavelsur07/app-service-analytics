...
const ADVERTISING_STATES: Record<string, ConnectionPresentation> = {
  active: {
    ...
    // Без обещания загрузки: загрузка рекламы появляется следующей стадией
    // (ADR-026, п. 5), и до неё «загружается» было бы неправдой.
    explanation: 'Рекламный ключ принят Ozon и сохранён.',
  },
  broken: {
    tone: 'negative',
...
export function advertisingPresentation(
  ...
  // Сломанный рекламный ключ показывается первым: восстановить подключение
  // магазина мало, ключ всё равно придётся заменить.
  if (state === 'active' && connectionState !== 'active') {
    return {
      tone: 'warning',
