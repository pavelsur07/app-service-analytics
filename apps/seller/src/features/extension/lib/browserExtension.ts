...
/**
 * Идентификатор расширения, которому будет отправлен токен.
 *
 * Выведен из ключа в apps/extension/manifest.config.ts — это одно
 * значение с двух сторон, и расходиться им нельзя: при несовпадении
 * приложение отправит токен несуществующему получателю и покажет
 * «расширение не приняло подключение».
 *
 * Меняется вместе с ключом после первой публикации в Chrome Web Store:
 * стор назначает расширению собственный идентификатор.
 */
const PINNED_EXTENSION_ID = 'nnglnfbkiklanbdmfhgflbjjckmmjlah'
