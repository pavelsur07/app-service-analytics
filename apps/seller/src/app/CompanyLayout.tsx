...
function CompanyShell({
  ...
  return (
    ...
    // В отличие от h-screen, fixed inset-0 вынимает оболочку из потока
    // документа. Это не даёт раскрытой динамике повторно создать scroll
    // у документа; единственным владельцем вертикального scroll остаётся
    // main. Горизонтальный overflow остаётся внутри main/table.
    //
    // min-h-0 на строке нужен, чтобы flex-ребёнок с рабочей зоной
    // сжался ниже содержимого и оставил scroll внутри main.
    <div className="fixed inset-0 flex flex-col overflow-hidden bg-bg-base">
