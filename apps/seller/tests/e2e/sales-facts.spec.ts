...
test.describe('sales facts', () => {
  ...
  test('seller sees Ozon sales facts imported from a real fixture', async ({
    ...
    // Августовские status-transition фикстуры экрана «Выкуп» новее этой
    // июльской выгрузки и занимают начало keyset-списка. Контрольная строка
    // теперь детерминированно находится на второй странице.
    await page.getByRole('button', { name: 'Дальше' }).click()
    // В исходном ответе есть две операции этой SKU с одинаковыми
    // отображаемыми полями; UI намеренно не схлопывает факты в одну строку.
    const knownRow = page
      .locator('tbody tr')
      .filter({ hasText: '4404411581' })
      .first()
