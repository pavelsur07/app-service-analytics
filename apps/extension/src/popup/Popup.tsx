    ...
    case 'connected':
      return (
        <>
          ...
          {/* Кнопка называется тем, что делает: стирает токен здесь.
              Отзыв на сервере — другое действие, и оно живёт в приложении
              (ADR-010): расширение ходит под bearer, а отзыв идёт под
              сессией, отсюда его не вызвать. Назвать это «Отключить»
              означало бы обещать прекращение доступа, которого не будет:
              утёкшая копия токена продолжит работать до истечения срока. */}
          <Button variant="secondary" onClick={onDisconnect}>
            Забыть на этом устройстве
          </Button>
          <p>Чтобы токен перестал действовать везде, отзовите его в Conwix.</p>
        </>
      )
