...
for chunk in "$WORK"/sku-chunk-*; do
    ...
    if [[ "$DRY_RUN" != true ]]; then
        ...
        covered=$(jq '[.items[].sku] | unique | length' "$TARGET")
        extra=$(jq -c 'del(.items) | keys' "$TARGET")
        printf '  пачка %s: SKU отправлено %s, SKU в ответе %s, строк %s, прочие ключи %s\n' \
            "$page_number" "$sent" "$covered" "$rows" "$extra"
        # Недостача SKU сама по себе не признак: товар без остатка может
        # честно отсутствовать. Подозрительны ключи продолжения и круглый
        # предел строк вместе с недостачей — это похоже на обрезку ответа.
        if [[ "$extra" != '[]' ]] || ((rows >= 1000 && rows % 1000 == 0 && covered < sent)); then
