...
for chunk in "$WORK"/sku-chunk-*; do
    ...
    if [[ "$DRY_RUN" != true ]]; then
        ...
        # Кратность 100 не признак: полная пачка из 100 SKU и так даёт
        # 100·k строк. Подозрительны ключи продолжения и круглое число
        # от 1000 — типичный предел ответа.
        if [[ "$extra" != '[]' ]] || ((rows >= 1000 && rows % 1000 == 0)); then
