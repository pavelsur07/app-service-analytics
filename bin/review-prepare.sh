...

{
    ...
    echo
    echo "Проверяй соответствие правилам и ADR из разделов 3-4 выше, а не"
    echo "предлагай общепринятые альтернативы (API Platform, findAll(), Redux"
    echo "для серверного состояния, Symfony Forms, глобальные фикстуры —"
    echo "здесь отвергнуты сознательно, см. ADR)."
    echo
