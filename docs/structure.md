...
В дереве выше перечислены только действующие папки. Решение о границах
`Planning` и `Inventory` принято в ADR-024, но их папки появятся с первой
задачей реализации. Для `Catalog`, `Costing` и `Finance` решения и папок
...
Будущий `Planning` хранит планы, импорт, настройки и историю расчётов;
заказы и исходы читает через узкий `IngestionPlanningFacade`, доступный
остаток — через `InventoryFacade`, принадлежность кабинета компании — через
...
срез через отдельный `IngestionStockSourceFacade`. Оба новых Facade Ingestion
получат отдельные слои Deptrac; существующий `IngestionFacade` останется
для PriceMonitoring. Код и Deptrac-слои этих модулей будут добавлены в
соответствующих задачах Этапа 0 (ADR-024).
