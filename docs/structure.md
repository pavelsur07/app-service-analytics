...
│                         веб, два Messenger-воркера и три служебных процесса
│                         из одного образа, Redis;
│                         не выкладка
...
томом, проксирование в php-fpm), `api`, два Messenger-воркера и три
служебных процесса из одного образа, Redis, PostgreSQL.
...
по TLS с сертификатами Let's Encrypt, сервисы действующей production-версии
`healthy`,
...
**Известное ограничение площадки.** Машина имеет 1.9 ГиБ памяти для
...
В дереве выше перечислены только действующие папки. `Planning` появился
с ручным дневным планом из этапа 0.2. Решение о границах `Inventory`
принято в ADR-024, но его папка появится с первой задачей реализации.
Для `Catalog`, `Costing` и `Finance` решения и папок
пока нет. Принятие ADR само по себе не создаёт пустой модуль.
...
Сбор наблюдений цены не зависит от `Ingestion`: цену приносит клиент.
Экран СПП читает историческую цену продавца через `IngestionFacade` —
это отдельная односторонняя связь `PriceMonitoring Application → Ingestion`
по ADR-016, уже отражённая в Deptrac.
...
`Planning` хранит планы и их аудит, а также XLSX preview/apply с временным
хранилищем, конкурентными блокировками и плановой очисткой. Настройки и
история расчётов будут добавляться следующими отдельными задачами. Заказы и исходы он
читает через узкий `IngestionPlanningFacade`, доступный остаток — через
будущий `InventoryFacade`, принадлежность кабинета компании — через
`IdentityAccountScopeFacade`, который не выдаёт ключи API. Будущий
`Inventory` хранит снимки доступного остатка и читает полный источниковый
срез через отдельный `IngestionStockSourceFacade`. `IngestionPlanningFacade`
уже имеет отдельный слой Deptrac; фасад источника остатков будет добавлен
вместе с Inventory. Существующий `IngestionFacade` остаётся для
PriceMonitoring (ADR-024).
