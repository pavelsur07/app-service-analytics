...
**Что проверено запуском.** Топология поднималась локально целиком
и обслуживала оба домена: маршруты через Traefik, статика, эндпоинты
API, редирект с http на https, оба воркера в состоянии `healthy`.
Не проверен выпуск сертификатов ACME — ему нужно публичное DNS-имя;
локально Traefik отдавал свой самоподписанный сертификат, то есть
проверен путь терминации TLS, но не получение сертификата.
