...
заказы и исходы читает через `IngestionFacade`, доступный остаток — через
`InventoryFacade`, принадлежность кабинета компании — через отдельный
`IdentityAccountScopeFacade`, который не выдаёт ключи API. Будущий `Inventory` хранит снимки доступного остатка и
читает полный источниковый срез через `IngestionFacade`. Код и Deptrac-слои
этих модулей будут добавлены в соответствующих задачах Этапа 0 (ADR-024).
