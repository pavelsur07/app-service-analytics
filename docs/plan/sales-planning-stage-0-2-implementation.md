...
| `api/src/Planning/Infrastructure/Repository/DoctrineDailyPlanRepository.php`, `Query/DailyPlansQuery.php` | Человеческая запись плана и аудита через одну ORM-транзакцию; дневное чтение через DBAL QueryBuilder и readonly DTO с нужными колонками |
| `api/src/Planning/Ui/Controller/`, `Ui/Request/` и `Ui/Response/` | `GET|PUT|DELETE /api/companies/{companyId}/planning/accounts/{accountId}/skus/{sku}/plan...`; строгие DTO запросов с Symfony Validator, seller OpenAPI, явные 422/404/409 |
| `api/composer.json`, `api/config/packages/validator.yaml` | Добавить Symfony Validator той же ветки `7.4.*`, что и остальные компоненты Symfony |
...
| `api/deptrac.php`, `api/bin/check-src-structure.sh`, `docs/structure.md` | Зарегистрировать Planning и узкие разрешённые связи к Identity/Ingestion, не открывая чужой Infrastructure |
...
`planning_daily_plan`: UUIDv7 PK и уникальный естественный ключ `(company_id, marketplace_account_id, marketplace_sku, business_date)`, `quantity INTEGER NULL CHECK (quantity >= 0)`, `version INTEGER NOT NULL CHECK (version > 0)`, `created_at`, `updated_at`, `updated_by`. Технический UUID нужен ORM: `date_immutable` нельзя использовать в составном идентификаторе Doctrine; уникальность предметного ключа сохраняет БД. `INTEGER` совпадает с типом `#[Version]` Doctrine. Строка после снятия остаётся с `quantity = NULL`; ранее не существовавший план читается как `{quantity:null, version:0}`. Ноль — установленный план. `planning_plan_change`: UUIDv7 PK, тот же естественный ключ, `old_quantity`, `new_quantity`, `old_version`, `new_version`, `actor_id`, `changed_at`; только добавление. Индексы начинаются с `company_id`, `marketplace_account_id`; `business_date` — дата заказа в `Europe/Moscow`.

`GET /.../plan?from&to`: включительные дни, последние 30 по умолчанию, максимум 90; вернуть каждый день периода, включая `null` и версию. `PUT /.../plan/{date}` принимает только `{quantity: целое от 0 до 2147483647, expectedVersion: целое от 0 до 2147483646}`. `DELETE /.../plan/{date}` принимает `{expectedVersion}` с той же верхней границей: версия `2147483647` уже не изменяема, потому что следующий optimistic-lock инкремент не помещается в PostgreSQL `INTEGER`. Создание нового дня требует `0`; восстановление снятого — сохранённую версию. Конфликт `409` возвращает актуальную версию/значение для UI. Дата/SKU/количество/период с ошибкой — `422`; чужой кабинет внутри компании — `404`; чужая компания — текущая политика членства. Проверка принадлежности кабинета через `IdentityAccountScopeFacade` проводится даже для пустого ряда, проверка известного SKU — через `IngestionPlanningFacade::knownMarketplaceSkus`.
